        ...
        let terminator = loop {
            ...
            match inst.op {
                ...
                Op::ExtInst => {
                    ...
                    if Some(set_id) == self.ext_non_semantic_id {
                    ...
                    } else if Some(set_id) == self.ext_debug_printf_id {
                        let op = self.next()?;
                        if op != 1 {
                            return Err(Error::UnsupportedExtInst(op));
                        }

                        let string_id = self.next()?;

                        let format = self.lookup_string.lookup(string_id)?.clone();

                        let mut arguments = Vec::new();
                        for _ in 0..inst.wc - 6 {
                            let arg_id = self.next()?;
                            let lexp = self.lookup_expression.lookup(arg_id)?;
                            arguments.push(get_expr_handle!(arg_id, lexp))
                        }
                        ...
                        let stmt = crate::Statement::DebugPrintf {
                            format,
                            arguments,
                        };
                        block.push(stmt, span);
